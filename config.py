...
# Used by the LLM Usage screen to estimate spend from logged token counts.
# Matching: exact "provider:model" first, then the longest substring pattern
# found in the model id, then "provider:*". Local models cost 0. Unknown
# models show "pricing unknown" in the UI — add a row here when you adopt a
# new provider/model (Anthropic prices per platform.claude.com, Jun 2026).
LLM_PRICING_USD_PER_MTOK = {
    ...
    "claude-opus-4":       (5.00, 25.00),   # opus 4.x family
    "claude-fable-5":      (10.00, 50.00),
    # Google Gemini (verify against current rate card when you switch)
    "gemini-2.5-pro":      (1.25, 10.00),
    "gemini-2.5-flash":    (0.30, 2.50),
    "gemini-2.0-flash":    (0.10, 0.40),
    # OpenAI / Azure OpenAI (verify against current rate card when you switch)
