...
Replaying the strategies over history without the replay telling itself the answer: lookahead
prevented at the seam, fills after the deciding bar, and every result carrying what it could
not account for.

Introduced by `backtesting` (see `openspec/archive/`).
