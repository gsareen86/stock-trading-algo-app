...
Who may use the platform: password login, JWT sessions that can actually be revoked, and a
guard that protects every endpoint by default.

Introduced by `authentication` (see `openspec/archive/`).
